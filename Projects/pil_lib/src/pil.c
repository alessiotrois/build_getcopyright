...
int	PIL_allow_spaces_cmndarg2value(const char *name, int numpar, char **result, int argc, char **argv)
 { char	buf[PIL_LINESIZE];
   ...
   for (i = 1; i < argc; i++)
    { if (NULL == argv[i]) continue;
      ...
      j = PIL_find_equal(buf);		/* find '=' in the string, neq. offset means not found */
      if (j < 0)
        {	/* we are here if no '=' has been found, continue with argv[i+1/i+2],
		2 cases are possible : argv[i+1]='=' and argv[i+2]=value,
		or : argv[i+1]='=value' */

          k = i + 1;			/* check if argv[i+1] is valid ... */
	  if (k < argc)  if (NULL != argv[k])  if (0 != argv[k][0])
	    {
	      if ('=' == argv[k][0])	/* so it should start with a '=' then */
	        { if (0 == numposarg) numposarg = i;  /* number of positional arguments + 1 */
                  if (0 == argv[k][1])	/* if it's single '=' char then argv[i+2] contains value */
	            { k = i + 2;	/* but ... let's check first if argv[i+2] exists at all */
	              if (k < argc)  if (NULL != argv[k])
	                {
		          if (strcmp(buf, name))  /* format found, but is it name we're looking for */
		            { i = k;	/* argv[i]=name, argv[i+1]='=', argv[i+2]=value */
		              continue;
		            }
	                  *result = argv[k];
	                  return(PIL_OK);
	            ...
	            { if (strcmp(buf, name))  /* format found, but is it name we're looking for */
		        { i = k;	/* argv[i]=name, argv[i+1]='=value' */
		        continue;
		        }
	              *result = &(argv[k][1]);
	              return(PIL_OK);
	...
	/* we are here only if  '=' has been found, see if the value is in argv[i] or argv[i+1] */
      buf[j] = 0;		/* since '=' has been found, name is only part of argv[i], mark its EOS */

      /* Note: positional arguments must precede 'name=value' arguments */
      if (0 == numposarg) {  /* check if this marks the end of the positional arguments */
           /* 
             We are here only if the command line argument contains an '='.  There are 2 possible cases: 
             1.  the name of a task parameter precedes the '='  (not a positional parameter)
             2.  or, the argument is a positional parameter, and the  '=' is part of a qualifying 
                 filter in a CFITSIO virtual file name. 
             In the 2nd case the parameter 'name' will include a '[' character, and in the first
             case the parameter name will not contain a '[' character.
           */
           if (!strchr(buf, '['))
               numposarg = i;  /* no '[' character, so this is not a positional argument */
      }

      if (strcmp(buf, name))  continue;	/* not the name we are looking for ... */

      if (0 != buf[j + 1])	/* if '=' is not the last char, then assume everything past it is the value */
        {
          *result = &(argv[i][j + 1]);
          return(PIL_OK);
        }
	/* we're here if argv[i]='name=', this means that argv[i+1] contains the value (if it exists) */
      k = i + 1;
      if (k < argc)  if (NULL != argv[k])  if (0 != argv[k][0])
        { *result = argv[k];
          return(PIL_OK);
   ...
   if (0 == numposarg) numposarg = argc;
   if ((numpar >= 1) && (numpar < numposarg))		/* positional arg: Value */
     { if (NULL == argv[numpar]) return(PIL_NOT_FOUND);
       *result = argv[numpar];
       return(PIL_OK);       
     }
   ...
   switch (PILCmndArgMode)
    { 
      case PIL_CMNDARG_ALLOW_SPACES:
		r = PIL_allow_spaces_cmndarg2value(pp->strname, numpar, &tmptr, argc, argv);
		break;
      default:
...
#endif
       strncpy(PILModuleName, p, PIL_LINESIZE);
       PILModuleName[PIL_LINESIZE - 1] = 0;
     }
   else
